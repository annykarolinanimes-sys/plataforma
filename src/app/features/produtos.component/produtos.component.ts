...
export class ProdutosComponent implements OnInit, OnDestroy {
  ...
  irParaNovo(): void {
    ...
    // Fetch next SKU from server and display it in the disabled field
    // show temporary placeholder while fetching
    try { this.ctrl('sku').setValue('A gerar...'); } catch {}
    this.svc.getNextSku().subscribe({
      next: sku => {
        console.log('Next SKU', sku);
        try { this.ctrl('sku').setValue(sku); } catch {}
      },
      error: err => {
        console.error('Erro getNextSku', err);
        this.errorMsg.set('Não foi possível gerar o SKU. Faça login novamente ou recarregue a página.');
        try { this.ctrl('sku').setValue(''); } catch {}
      }
    });
  }
  ...
  salvarProduto(): void {
    ...
    if (this.isEditView() && this.editingId()) {
      ...
      this.svc.criar(dto).subscribe({
        next:  produto => this._onCreateSuccess(produto, 'Produto criado com sucesso.'),
        error: e  => this._onSaveError(e.message),
      });
    }
  }
  ...
  private _onCreateSuccess(produto: ProdutoModel, msg: string): void {
    this.isSaving.set(false);
    this.irParaDetalhes(produto);
    this.showToast(msg);
  }
