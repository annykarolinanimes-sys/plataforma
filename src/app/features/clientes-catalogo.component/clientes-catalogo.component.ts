...
export class ClientesCatalogoComponent implements OnInit, OnDestroy {
  ...
  goToCreate(): void {
    ...
    // show temporary placeholder and fetch next cliente code
    try { this.ctrl('codigo').setValue('A gerar...'); } catch {}
    this.svc.getNextCodigo().subscribe({
      next: code => {
        console.log('Next cliente code', code);
        try { this.ctrl('codigo').setValue(code); } catch {}
      },
      error: err => {
        console.error('Erro getNextCodigo cliente', err);
        this.errorMsg.set('Não foi possível gerar o código do cliente. Faça login novamente ou recarregue a página.');
  ...
  salvarCliente(): void {
    ...
    if (this.isEditing() && this.editingId()) {
      ...
      this.svc.criar(dto).subscribe({
        next:  cliente => this._onCreateSuccess(cliente, 'Cliente criado com sucesso.'),
        error: e  => this._onSaveError(e.message),
      });
    }
  }
  ...
  private _onCreateSuccess(cliente: ClienteModel, msg: string): void {
    this.isSaving.set(false);
    this._patchCliente(cliente);
    this.uiState.goToClienteDetails(cliente.id);
    this.showToast(msg);
