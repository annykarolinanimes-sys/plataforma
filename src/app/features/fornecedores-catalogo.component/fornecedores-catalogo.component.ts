...
export class FornecedoresCatalogoComponent implements OnInit, OnDestroy {
  ...
  goToCreate(): void {
    ...
    // show placeholder and fetch next fornecedor code
    try { this.ctrl('codigo').setValue('A gerar...'); } catch {}
    this.svc.getNextCodigo().subscribe({
      next: code => {
        console.log('Next fornecedor code', code);
        try { this.ctrl('codigo').setValue(code); } catch {}
      },
      error: err => {
        console.error('Erro getNextCodigo fornecedor', err);
        this.errorMsg.set('Não foi possível gerar o código do fornecedor. Faça login novamente ou recarregue a página.');
        try { this.ctrl('codigo').setValue(''); } catch {}
      }
    });
  }
  ...
  salvarFornecedor(): void {
    ...
    if (this.uiState.isFornecedorEdit() && this.editingId()) {
      ...
      this.svc.criar(base).subscribe({
        next: fornecedor => this._onCreateSuccess(fornecedor, 'Fornecedor criado com sucesso.'),
        error: e  => this._onSaveError(e.message),
      });
    }
  }
  ...
  private _onCreateSuccess(fornecedor: FornecedorModel, msg: string): void {
    this.isSaving.set(false);
    this._patchFornecedor(fornecedor);
    this.uiState.goToFornecedorEdit(fornecedor.id);
    this.isViewing.set(true);
