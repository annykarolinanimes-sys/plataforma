...
export class RecepcaoComponent implements OnInit, OnDestroy {
  ...
  fornecedorSearchTerm = signal('');
  showFornecedorDropdown = signal(false);
  filteredFornecedores = computed(() => {
    const term = this.fornecedorSearchTerm().trim().toLowerCase();
    if (term.length < 1) return [] as FornecedorModel[];
    
    return this.fornecedores()
      .filter(f => {
        const idFormatado = f.id.toString().padStart(3, '0');
        return (
          f.nome.toLowerCase().includes(term) ||
          (f.nif?.toLowerCase().includes(term) ?? false) ||
          (f.telefone?.toLowerCase().includes(term) ?? false) ||
          idFormatado.includes(term) ||
  ...
  produtoSearchTerms = signal<string[]>([]);
  produtoDropdowns = signal<boolean[]>([]);

  // Signals para o dropdown global
  dropdownPosition = signal<{ top: number; left: number; width: number } | null>(null);
  activeDropdownIndex = signal<number | null>(null);
  cancelDropdownClose = false;
  ...
  adicionarItem(): void {
    ...
    this.produtoSearchTerms.update(term => [...term, '']);
    this.produtoDropdowns.update(dropdown => [...dropdown, false]);
    this._recalcularTotais();
  }

  removerItem(index: number): void {
    this.itensArray.removeAt(index);
    this.produtoSearchTerms.update(term => term.filter((_, i) => i !== index));
    this.produtoDropdowns.update(dropdown => dropdown.filter((_, i) => i !== index));
    if (this.itensArray.length === 0) this.adicionarItem();
    this.alertaDuplicado.set(null);
    this._recalcularTotais();
  }

  getFornecedorSelecionado(): FornecedorModel | null {
    const id = this.form.get('fornecedorId')?.value;
    return this.fornecedores().find(f => String(f.id) === String(id)) ?? null;
  }

  getProdutoSelecionado(index: number): ProdutoModel | null {
    const produtoId = this.itensArray.at(index).get('produtoId')?.value;
    return this.produtos().find(p => String(p.id) === String(produtoId)) ?? null;
  }

  getProdutoSearchTerm(index: number): string {
    return this.produtoSearchTerms()[index] ?? '';
  }

  onFornecedorSearch(value: string): void {
    const term = value || '';
    this.fornecedorSearchTerm.set(term);
    this.showFornecedorDropdown.set(term.trim().length >= 2 && this.filteredFornecedores().length > 0);
    this.form.patchValue({ fornecedorId: '' });
  }

  onFornecedorFocus(): void {
    this.showFornecedorDropdown.set(this.filteredFornecedores().length > 0);
  }

  closeFornecedorDropdown(): void {
    setTimeout(() => this.showFornecedorDropdown.set(false), 120);
  }

  selecionarFornecedor(fornecedor: FornecedorModel): void {
    this.form.patchValue({ fornecedorId: fornecedor.id });
    this.fornecedorSearchTerm.set('');
    this.showFornecedorDropdown.set(false);
  }
  ...
  onProdutoSearch(index: number, value: string, event?: Event): void {
    const term = value || '';
    this.produtoSearchTerms.update(list => list.map((v, i) => i === index ? term : v));
    
    if (term.trim().length >= 2 && this.getFilteredProdutos(index).length > 0) {
      this.calculateDropdownPosition(index, event);
  ...
  calculateDropdownPosition(index: number, event?: Event): void {
    ...
    if (event && event.target) {
      ...
      if (row) {
        inputElement = row.querySelector('.item-search-input') as HTMLElement;
      }
    }
  ...
  closeProdutoDropdown(index: number): void {
    setTimeout(() => {
      if (!this.cancelDropdownClose && this.activeDropdownIndex() === index) {
        this.activeDropdownIndex.set(null);
        this.dropdownPosition.set(null);
      }
      this.cancelDropdownClose = false;
    }, 150);
  }

  getFilteredProdutos(index: number): ProdutoModel[] {
    const term = this.getProdutoSearchTerm(index).trim().toLowerCase();
  ...
  selecionarProduto(index: number, produto: ProdutoModel): void {
    this.itensArray.at(index).get('produtoId')?.setValue(produto.id);
    this.produtoSearchTerms.update(list => list.map((v, i) => i === index ? '' : v));
    this.produtoDropdowns.update(list => list.map((v, i) => i === index ? false : v));
    this.onProdutoChange(index, String(produto.id));
  }

  selecionarProdutoGlobal(index: number, produto: ProdutoModel, event: Event): void {
  ...
  limparProduto(index: number): void {
    this.itensArray.at(index).get('produtoId')?.setValue('', { emitEvent: false });
    this.produtoSearchTerms.update(list => list.map((v, i) => i === index ? '' : v));
    this.produtoDropdowns.update(list => list.map((v, i) => i === index ? false : v));
    this.alertaDuplicado.set(null);
  ...
  private _resetForm(): void {
    ...
    this.produtoSearchTerms.set([]);
    this.produtoDropdowns.set([]);
    this.form.reset({ fornecedorId: '', tipoEntrada: 'Fornecedor', prioridade: 'Media' });
    this.adicionarItem();
    this.fornecedorSearchTerm.set('');
    this.showFornecedorDropdown.set(false);
    this.errorMsg.set(null);
  ...
  private _patchRecepcao(recepcao: Recepcao): void {
    ...
    this.produtoSearchTerms.set(Array(this.itensArray.length).fill(''));
    this.produtoDropdowns.set(Array(this.itensArray.length).fill(false));
    if (this.itensArray.length === 0) this.adicionarItem();
    this.fornecedorSearchTerm.set('');
    this.showFornecedorDropdown.set(false);
    this.errorMsg.set(null);
