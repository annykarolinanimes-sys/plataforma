...
export class UserService {
  ...
  listarMotoristas(transportadoraId?: number, search?: string, ativo?: boolean): Observable<MotoristaDto[]> {
    ...
    const url = params.toString() ? `${this.api}/motoristas?${params}` : `${this.api}/motoristas`;
    console.log('Chamando API de motoristas:', url);
    return this.http.get<MotoristaDto[]>(url).pipe(
      map(response => {
        console.log('Resposta da API de motoristas:', response);
        return response || [];
      })
    );
  }
  ...
  obterMotoristaPorTransportadora(transportadoraId: number): Observable<MotoristaDto[]> {
    const url = `${this.api}/motoristas?transportadoraId=${transportadoraId}`;
    return this.http.get<MotoristaDto[]>(url);
  }

  criarMotorista(data: CreateMotoristaRequest): Observable<MotoristaDto> {
    return this.http.post<MotoristaDto>(`${this.api}/motoristas`, data);
  }

  atualizarMotorista(id: number, data: UpdateMotoristaRequest): Observable<MotoristaDto> {
    return this.http.put<MotoristaDto>(`${this.api}/motoristas/${id}`, data);
  }

  eliminarMotorista(id: number): Observable<void> {
    return this.http.delete<void>(`${this.api}/motoristas/${id}`);
  }

  ativarMotorista(id: number): Observable<{ message: string }> {
    return this.http.post<{ message: string }>(`${this.api}/motoristas/${id}/ativar`, {});
  }
}