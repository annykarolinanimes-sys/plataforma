...
export interface AlterarMatriculaResponse {
  message: string;
  novaMatricula: string;
}

export interface VeiculoAnexo {
  id: number;
  nomeOriginal: string;
  mimeType: string;
  tamanhoBytes: number;
...
export class VeiculosService {
  ...
  /**
   * Altera a matrícula de um veículo.
   * Envia multipart/form-data com: novaMatricula, motivo e o ficheiro comprovativo.
   */
  alterarMatricula(
    id: number,
    novaMatricula: string,
    motivo: string,
    documento: File
  ): Observable<AlterarMatriculaResponse> {
    const formData = new FormData();
    formData.append('novaMatricula', novaMatricula);
    formData.append('motivo',        motivo);
    formData.append('documento',     documento, documento.name);

    return this.http
      .post<AlterarMatriculaResponse>(`${this.api}/${id}/change-matricula`, formData)
      .pipe(catchError(this.handleError));
  }

  uploadAnexo(id: number, ficheiro: File): Observable<VeiculoAnexo> {
  const formData = new FormData();
  formData.append('ficheiro', ficheiro);
  return this.http.post<VeiculoAnexo>(`${this.api}/${id}/anexos`, formData)
    .pipe(catchError(this.handleError));
  }

  downloadAnexo(id: number, anexoId: number): Observable<Blob> {
    return this.http.get(`${this.api}/${id}/anexos/${anexoId}`, {
      responseType: 'blob'
    }).pipe(catchError(this.handleError));
  }

  removerAnexo(id: number, anexoId: number): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.api}/${id}/anexos/${anexoId}`)
      .pipe(catchError(this.handleError));
  }
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    ...
    else if (error.status === 400) message = error.error?.message ?? 'Dados inválidos.';
