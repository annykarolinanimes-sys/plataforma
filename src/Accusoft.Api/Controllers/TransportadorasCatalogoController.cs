...
public class TransportadorasCatalogoController : ControllerBase
{
        ...
        [FromQuery] string orderDir = "asc")
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(t =>
                (idFiltro.HasValue && t.Id == idFiltro.Value) ||
                (t.Nome != null && t.Nome.ToLower().Contains(s)) ||
                (t.Codigo != null && t.Codigo.ToLower().Contains(s)) ||
                (t.Nif != null && t.Nif.ToLower().Contains(s)) ||
                (t.Telefone != null && t.Telefone.ToLower().Contains(s)) ||
                (t.Email != null && t.Email.ToLower().Contains(s)) ||
                (t.Localidade != null && t.Localidade.ToLower().Contains(s)) ||
                (t.CodigoPostal != null && t.CodigoPostal.ToLower().Contains(s)) ||
                (t.Pais != null && t.Pais.ToLower().Contains(s)) ||
                (t.ContactoNome != null && t.ContactoNome.ToLower().Contains(s)) ||
                (t.ContactoTelefone != null && t.ContactoTelefone.ToLower().Contains(s)) ||
                (t.Observacoes != null && t.Observacoes.ToLower().Contains(s)));
        }
    ...
    [HttpGet("next")]
    public async Task<IActionResult> GetNextTransportadoraCodigoEndpoint()
    {
        var uid = User.GetUserId();
        var codigo = await GetNextTransportadoraCodigo(uid);
    ...
    public async Task<IActionResult> CreateTransportadora([FromBody] TransportadoraCreateDto dto)
    {
        ...
        {
            var normalizedNif = dto.Nif.Trim().ToLowerInvariant();

            if (await _db.TransportadorasCatalogo.AnyAsync(t =>
                    t.Nif != null && t.Nif.ToLower() == normalizedNif))
                return Conflict(new { message = "Já existe uma transportadora com este NIF." });

            if (await _db.FornecedoresCatalogo.AnyAsync(f =>
                    f.Nif != null && f.Nif.ToLower() == normalizedNif))
    ...
    public async Task<IActionResult> UpdateTransportadora(int id, [FromBody] TransportadoraUpdateDto dto)
    {
        ...
        {
            var normalizedNif = dto.Nif.Trim().ToLowerInvariant();
            var currentNif = transportadora.Nif?.Trim().ToLowerInvariant();

            if (currentNif != normalizedNif)
            {
                if (await _db.TransportadorasCatalogo.AnyAsync(t =>
                        t.Nif != null && t.Nif.ToLower() == normalizedNif && t.Id != id))
