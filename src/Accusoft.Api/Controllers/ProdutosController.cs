...
public class ProdutosController : ControllerBase
{
    ...
    // Método auxiliar para gerar SKU único
    private async Task<string> GerarSkuUnicoAsync(int userId)
    {
        var prefix = "SKU";
        var lastNumber = 0;
        
        // Buscar o último SKU criado pelo usuário
        var ultimoSku = await _db.Produtos
            .Where(p => p.CriadoPor == userId && p.Sku.StartsWith(prefix))
            .OrderByDescending(p => p.Id)
            .Select(p => p.Sku)
            .FirstOrDefaultAsync();

        if (!string.IsNullOrEmpty(ultimoSku))
        {
            var parts = ultimoSku.Split('-');
    ...
    public async Task<IActionResult> CreateProduto([FromBody] ProdutoCreateDto dto)
    {
        ...
        // Se o SKU foi fornecido, validar unicidade
        ...
        if (dto.FornecedorId.HasValue)
        {
            var fornecedor = await _db.FornecedoresCatalogo
                .FirstOrDefaultAsync(f => f.Id == dto.FornecedorId.Value && f.CriadoPor == uid && f.Ativo);
            if (fornecedor == null)
                return BadRequest(new { message = "Fornecedor inválido ou inativo." });
        }

        // Validar stock mínimo
        if (dto.StockMinimo > dto.StockInicial)
            return BadRequest(new { message = "Stock mínimo não pode ser maior que o stock inicial." });

        // Validar localização se fornecida
        if (!string.IsNullOrWhiteSpace(dto.Localizacao))
        {
            var localizacaoValida = await _db.ArmazensCatalogo
                .AnyAsync(a => a.Localizacao == dto.Localizacao.Trim() && a.CriadoPor == uid);
            if (!localizacaoValida)
                return BadRequest(new { message = "Localização inválida. Deve estar registada na aba Armazéns." });
        }

        var now = DateTimeOffset.UtcNow;
        
        // Gerar SKU automaticamente se não foi fornecido
        var sku = string.IsNullOrWhiteSpace(dto.Sku) 
            ? await GerarSkuUnicoAsync(uid) 
            : dto.Sku.Trim();
